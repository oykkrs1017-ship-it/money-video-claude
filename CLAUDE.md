...
npx remotion studio src/index.ts --props=input/script-input.json  # Remotion Studio起動
...
# ep生成パイプライン（手動個別実行 — 通常は new-ep 統合パイプラインが
# Step 1〜4.5 と script-input.json 同期を自動実行する。以下は個別再実行用）
# 1. 台本生成
...
## ai-money-shorts 固有注意事項（packages/_archived/ に退避済み — 旧パッケージを触る場合のみ参照）
